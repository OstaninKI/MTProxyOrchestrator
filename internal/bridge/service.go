...
func (s *BridgeService) Enable(cfg EnableConfig) error {
	...
	if cfg.SingboxURL != "" {
		if err := s.Exec.Download(cfg.SingboxURL, cfg.SingboxSHA256, cfg.Paths.SingboxBin); err != nil {
			return fmt.Errorf("bridge enable: download sing-box: %w", err)
		}
	}

	// 3. Render and write sing-box.json.
	active := nl.Active()
	outbounds := make([]singbox.VLESSOutbound, 0, len(active))
	for _, n := range active {
		outbounds = append(outbounds, singbox.VLESSOutbound{
			Tag:       n.Tag,
			Server:    n.Host,
			Port:      n.Port,
			UUID:      n.UUID,
	...
	if err != nil {
		return fmt.Errorf("bridge enable: render sing-box.json: %w", err)
	}
	if err := s.Exec.WriteFile(cfg.Paths.SingboxJSON, sbJSON, 0o600); err != nil {
		return fmt.Errorf("bridge enable: write sing-box.json: %w", err)
	}
	...
	if err := s.Exec.WriteFile(cfg.Paths.SingboxService, sbUnit.Render(), 0o644); err != nil {
		return fmt.Errorf("bridge enable: write sing-box.service: %w", err)
	}
	...
	if err := s.Exec.WriteFile(cfg.Paths.TeleproxyTOML, tpCfg.Render(), 0o600); err != nil {
		return fmt.Errorf("bridge enable: write teleproxy.toml: %w", err)
	}

	// 6. Enable and start sing-box, then restart teleproxy.
	if err := s.Exec.EnableService("sing-box.service"); err != nil {
		return fmt.Errorf("bridge enable: enable sing-box: %w", err)
	}
	if err := s.Exec.StartService("sing-box.service"); err != nil {
		return s.rollback(cfg, fmt.Errorf("bridge enable: start sing-box: %w", err))
	}
	if err := s.Exec.StartService("teleproxy.service"); err != nil {
		return s.rollback(cfg, fmt.Errorf("bridge enable: restart teleproxy: %w", err))
	}

	// 7. Health check: both services must be active.
	sbActive, err := s.Exec.ServiceActive("sing-box.service")
	if err != nil || !sbActive {
		return s.rollback(cfg, fmt.Errorf("bridge enable: sing-box not active after start"))
	}
	tpActive, err := s.Exec.ServiceActive("teleproxy.service")
	if err != nil || !tpActive {
		return s.rollback(cfg, fmt.Errorf("bridge enable: teleproxy not active after restart"))
	}
...
func (s *BridgeService) rollback(cfg EnableConfig, cause error) error {
	tpCfg := teleproxy.Config{
		Port:      cfg.MTProtoPort,
		MaskHost:  cfg.MaskHost,
		StatsPort: cfg.StatsPort,
	...
	_ = s.Exec.WriteFile(cfg.Paths.TeleproxyTOML, tpCfg.Render(), 0o600)
	_ = s.Exec.StopService("sing-box.service")
	_ = s.Exec.DisableService("sing-box.service")
	_ = s.Exec.StartService("teleproxy.service")
